      ...
      <div className="container mx-auto px-4 relative">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed">
            Dr. Praveen's commitment to excellence is reflected in every
            surgery, every patient interaction, and every innovative technique
            adopted.
          </p>
        </div>
          ...
          {achievements.map((achievement, index) => (
            ...
            >
              <CardContent className="p-8 text-center relative overflow-hidden">
                {/* Background gradient */}
                <div
                  className={`absolute inset-0 bg-gradient-to-br ${achievement.bgGradient} opacity-0 group-hover:opacity-100 transition-opacity duration-300`}
                ></div>

                <div className="relative z-10">
                  <div
                    className={`w-16 h-16 bg-gradient-to-br ${achievement.gradient} rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-lg group-hover:scale-110 transition-transform duration-300`}
                  >
                    <achievement.icon className="h-8 w-8 text-white" />
                  </div>

                  {/* --- MODIFIED LINE --- */}
                  {/* I've added h-12, flex, items-center, and justify-center to fix the alignment */}
                  <div className="h-10 flex items-center justify-center text-3xl lg:text-4xl font-bold mb-2 bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent">
                    {achievement?.number}
                  </div>
